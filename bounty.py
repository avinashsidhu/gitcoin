...
# user github repo: #repos, how active, tenure
...
_vars = [
        ...
        'url', 'github_url', 'is_open', 'status', 'network',
    ]
...
class Bounty:
    ...
    def __post_init__(self):
        ...
        self.fulfillment = self._update('fulfillment')
    ...
    def _update(self, event_type):
        ...
        for event in events:
            event_obj = eventclass(**{'pk':self.pk}, **event)
            event_obj.update()
    ...
    def get_owner_attr(self, method):
        if self.org_name:
            date = self.created
            return getattr(OwnerTally(Owner(self.org_name), date), method)

    def get_user_attr(self, method):
        if self.interest:
            date = self.created
            attrs = [getattr(UserTally(User(applicant.handle), date), method) for applicant in self.interest]
            return np.mean(attrs)
            # return [getattr(UserTally(User(handle), date), method) for handle in self.paid]
