...
class Proxy:
    ...
    ERR_PATH = get_path(f'var/tmp/gitcoin_api_err_{now()}.log')
    ...
    def get(self, url):
        ...
        try:
            ...
            return Results(url, config_id, response)
            ...
            logging.warning(message)
            return Results(url, None, None)


class Results():
    """Results from Proxy.get(). Similar to Response to requests package."""

    def __init__(self, url, config, response):
        self.url = url
        self.config = config
        self.time = now()
